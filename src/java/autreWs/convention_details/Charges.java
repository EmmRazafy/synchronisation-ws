...
public class Charges {
    ...
    private final static List<List<String>> ALL_SYNCHRO_NAME = Factures.ALL_SYNCHRO_NAME;
    private final static String CONVENTION_SYNCHRO_NAME = Factures.CONVENTION_SYNCHRO_NAME;//Convention
    private final static String SYNCHRO_NAME = ALL_SYNCHRO_NAME.get(4).get(1);//Facture
    private final static HashMap<String, Integer> DATA_INDICE/*etblissmt_data_indice*/ = HashHelper.newHashMap(
        new String[] {"synchro_id", "desc", "total", "email", "last_edit_date",  "conflit_cent_synchro_id", "t_synchro_date_reception", "t_synchro_date_envoye"},/**/
    ...
    public static List<HashMap<String,Object>[]> getPageData(long conventionId, HashMap<String, Object> neededData, String stationCibleType, Connection connection, long pagntStartNum, int pagntMaxSize) throws ParamException, Exception{
        ...
        String sqlR = (!isPeripheriqueStationType)
            ...
            "       last_edit_date\n" +
            "    from (\n" +
            "            SELECT description, t_user_id, t_synchro_date_edition AS last_edit_date, t_synchro_id, COALESCE(SUM(MTT), 0)::MONTANT AS total_mtt\n" +
            "            FROM t_cent_synchro_charge \n" +
            "            WHERE t_convention_id = "+conventionId+" AND t_synchro_date_suppr IS NULL\n" +
            "            GROUP BY description, t_user_id, t_synchro_date_edition, t_synchro_id \n" +
            "            ORDER BY t_synchro_date_edition DESC , t_synchro_id\n" +
            ...
            "from (\n" +
            "         SELECT description, t_user_id, t_synchro_date_edition AS last_edit_date, t_synchro_id, COALESCE(SUM(MTT), 0)::MONTANT AS total_mtt "+SynhronisationCols+" \n" +/**/
            "         FROM t_peri_synchro_charge\n" +
            "         WHERE t_convention_id = "+conventionId+" AND t_synchro_date_suppr IS NULL\n" +
            "         GROUP BY description, t_user_id, t_synchro_date_edition, t_synchro_id \n" +
            "         ORDER BY t_synchro_date_edition DESC , t_synchro_id\n" +
            "            OFFSET "+pagntStartNum+" LIMIT "+pagntMaxSize+" \n" +
        ...
        Key[] orderedKeys = (isPeripheriqueStationType)?/**/
                new Key[]{new Key<>("synchro_id", Long.class), new Key<>("description", String.class), new Key<>("total_mtt", BigDecimal.class, BigDecimal.ZERO), new Key<>("email", String.class), new Key<>("last_edit_date", Timestamp.class)     , new Key<>("conflit_cent_synchro_id", String.class), new Key<>("t_synchro_date_reception", Timestamp.class), new Key<>("t_synchro_date_envoye", Timestamp.class)}
            :   new Key[]{new Key<>("synchro_id", Long.class), new Key<>("description", String.class), new Key<>("total_mtt", BigDecimal.class, BigDecimal.ZERO), new Key<>("email", String.class), new Key<>("last_edit_date", Timestamp.class)};
    ...
    public static <SynchroType extends Synchro> HashMap<String, Object> getData(long conventionId, HashMap<String, Object> hmapResponse, String stationCibleType, Connection connection) throws SQLException {
        ...
        String[] tabName = new String[]{"the_cent_timezone" , "t_cent_synchro_charge", "t_cent_synchro_convention", "t_cent_synchro_ann_sclr", "t_cent_synchro_lieu_affectation", "t_cent_synchro_type_etablissmt", "t_cent_synchro_type_enseignmt", "t_cent_synchro_niveau_enseignmt"};
        ...
        String sqlR = ""
            ...
            "    COALESCE(total_mtt, 0)::MONTANT AS total_mtt \n" +
            "FROM\n" +
            "     get_this_cent_peri_station_timezone() t_station_timezone CROSS JOIN\n" +
            "    (SELECT "+conventionId+" AS convention_id, SUM(MTT) AS total_mtt, count(*) AS nbr_ligne from "+tabName[i++]+" where t_synchro_date_suppr IS NULL AND t_convention_id = "+conventionId+") t1\n" +
        ...
        Key[] keys = new Key[]{
            ...
            new Key<>("total_mtt", BigDecimal.class, BigDecimal.ZERO)//12
        };
